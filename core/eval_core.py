"""Hardened selectivity-eval primitives (Hewitt-Liang control-task logic).

Canonical source = ``phase1/eval_opcycle.py``. ``phase1_5/eval.py`` re-copied these
verbatim (its phase1_5-specific adapters — ``make_phase15_controls``,
``selectivity_gate_phase15``, ``regate_operation_selectivity``, etc. — stay in that
module and import these primitives from here). Behaviour is byte-identical.

Hewitt-Liang (2019) control-task logic: a representation is *selective* for a property
only if a linear probe decodes that property far better than it decodes a control. The
sequence code is ``mean_t α`` (per-token ReMoE gates, masked-averaged); we compare
acc(operation) against random / topic / token / geometry controls.
